...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
    ...
    $html_str .= '.labelHeaderStyle { background-color: #ffffff; font-weight: bold; padding: 5px; margin-bottom: 5px; font-size: 11pt;}'."\n";
    $html_str .= '.labelSubHeaderStyle { background-color: #ffffff; font-weight: regular;padding: 0px; margin-bottom: 0px; font-size: 8pt;}'."\n";
    ...
    foreach ($chunked_label_arrays as $label_data) {
        ...
        foreach ($label_data as $label) {
            ...
            if ($sysconf['print']['label']['include_header_text']) { $html_str .= '<div class="labelHeaderStyle">'.($sysconf['print']['label']['header_text']?$sysconf['print']['label']['header_text']:$sysconf['library_name']);
                $html_str .= '<div class="labelSubHeaderStyle">' .__('UPT PELATIHAN KOPERASI DAN UKM PROVINSI JAWA TIMUR').'</div>'.'</div>'; }
                $html_str .= '<hr style="
                            position: relative;
                            border: none;
                            height: 3px;
                            background: black;
                            margin-bottom: 15px;
                        " />';//manambah garis di bawah header
